...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    display.display_netpbm(&image_copy)?;
    ...
    let a0_pgm = pgm_from_float_image(&a.0);
    let a1_pgm = pgm_from_float_image(&a.1);
    display.display_netpbm(&a0_pgm)?;
    display.display_netpbm(&a1_pgm)?;
    let mut reconstructed = subband_coder.synthesis(&a.0, &a.1)?;
    reconstructed.find_and_set_min_max();
    let reconstructed = pgm_from_float_image(&reconstructed);
    display.display_netpbm(&reconstructed)?;
